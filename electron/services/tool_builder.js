// ============ 工具定义构建器（Electron 主进程版） ============
// 迁移自 chrome-extension/background/services/agent-tool-builder.js
// 构建统一的 LLM 工具定义列表（OpenAI Function Calling 格式）
// 依赖：scriptService（域名过滤）、filteredScriptsCache、domainMismatchLogged
// 改动：
//   - ES Module → CommonJS (module.exports / require)
//   - report-templates.js 的 getTemplateList → 内置模板列表（_getTemplateList）
//     （Electron 暂无独立 report-templates 模块，后续可替换为远程加载）
//
// 策略：单阶段全量暴露（非分阶段）。所有工具在每轮都可用。

/**
 * 内置报告模板精简列表（用于 render_report 工具描述）
 * 迁移自 chrome-extension/shared/report-templates.js 的 BUILTIN_TEMPLATES
 * 仅保留 id/name/description/fields，供 AI 选择模板参考
 * 后续可替换为从后端加载的远程模板
 */
...
/**
 * 获取所有模板的精简列表（用于工具描述）
 * 对应 chrome-extension 的 getTemplateList()
 * @returns {Array<{id, name, description, fields}>}
 */
...
/**
 * 过滤并缓存域名匹配的脚本
 * 按 urlPattern 域名过滤，最多保留 6 个
 * @param {Array} searchResults - 搜索到的脚本列表
 * @param {string} currentPageUrl - 当前页面 URL
 * @param {object} scriptService - 脚本服务（提供 matchUrl 方法）
 * @param {Map} filteredScriptsCache - 已过滤脚本缓存
 * @param {Set} domainMismatchLogged - 已记录的域名不匹配日志（避免重复打印）
...
/**
 * 按经验记忆排序并构建脚本工具定义
 * 成功率高的脚本排在前面，最多构建 6 个 inject_script_N 工具
 * @param {Array} scripts - 过滤后的脚本列表
...
/**
 * 构建统一工具定义（单阶段，所有工具可用）
 * 包含 DOM 工具(17个) + 脚本工具(动态N个) + 高级工具(5个)
 * @param {Array} searchResults - 搜索到的脚本列表
 * @param {string} currentPageUrl - 当前页面 URL
 * @param {number} round - 当前轮次（保留参数，单阶段策略下不再用于分阶段）
 * @param {object} scriptService - 脚本服务（提供 matchUrl 方法）
 * @param {Map} filteredScriptsCache - 已过滤脚本缓存
 * @param {Set} domainMismatchLogged - 已记录的域名不匹配日志
 * @returns {Array} OpenAI Function Calling 格式的工具定义列表
 */
function buildTools(searchResults, currentPageUrl, round, scriptService, filteredScriptsCache, domainMismatchLogged) {
  const tools = []
  ...
  tools.push({
    ...
    function: {
      name: 'fetch_url',
      description: '在主进程发起网络请求，突破页面 CORS 限制。适用于跨域获取 HTML/JSON 数据。返回响应文本（字符串）。如需解析 HTML，可将结果存入 data_refs 后用 generate_script 配合 DOMParser 处理（注意 CSP 限制）。',
      parameters: {
        type: 'object',
        properties: {
  ...
  // 模板从内置列表加载，后续可扩展为从后端加载
  const _templateList = _getTemplateList()
...
// 保留旧函数名作为别名（兼容期）
// 单阶段策略下，phase1/phase2 均调用 buildTools
function buildPhase1Tools(currentPageUrl, round, scriptService, filteredScriptsCache, domainMismatchLogged) {
  return buildTools([], currentPageUrl, round, scriptService, filteredScriptsCache, domainMismatchLogged)
}

function buildPhase2Tools(searchResults, currentPageUrl, round, scriptService, filteredScriptsCache, domainMismatchLogged) {

// ============ PayloadStore 工具函数（Electron 主进程版） ============
// 迁移自 chrome-extension/background/services/agent-payload-utils.js
// 纯函数模块，无外部依赖，供 agent_runner.js 调用
// 改动：ES Module → CommonJS (module.exports / require)

/**
 * 从数据中自动检测 schema（字段名→类型）
 * 取前2条数据合并字段，生成类型映射
 */
function autoDetectSchema(items) {
 ...
 */
function extractSamples(items, maxFieldLen = 60) {
 ...
 * 所有数据工具的输出经此包装后格式一致
 */
function normalizePayload(raw, toolName) {
  // 已经是标准信封格式
 ...
 */
function formatSchemaSummary(entryId, toolName, envelope) {
 ...
 * 关键：大数据存储，小数据智能截断
 */
function shouldStoreToPayload(result, toolName) {
  // 这些工具结果不需要存储（查询类/搜索类）
 ...
 * @param {object} payloadStore - 数据存储对象，需提供 add(toolName, result, summary, metadata) 方法
 * @param {*} result - 工具原始结果
 * @param {string} toolName - 工具名称
 * @param {object} envelope - normalizePayload 生成的信封对象
 * @returns {Promise<string>} 返回给 AI 的摘要文本
 */
async function storeToPayload(payloadStore, result, toolName, envelope) {
 ...
 */
function smartTruncateResult(result, maxLen = 2000) {
  // 统一为字符串处理
 ...
 */
function buildDataOverview(result, toolName) {
  try {
    const obj = typeof result === 'string' ? JSON.parse(result) : result

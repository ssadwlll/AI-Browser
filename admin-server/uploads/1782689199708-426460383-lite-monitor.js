// @name: Lite套餐补货监控
// @description: 定时监控Lite套餐购买按钮状态，补货时自动通知。每30秒检查一次，最长运�?小时�?// @version: 1.0.0
// @urlPattern: *

// 监控面板显示脚本 - 监听 Lite 套餐补货状�?// 创建监控面板 UI
const panel = document.createElement('div');
panel.id = 'monitor-panel';
panel.style.cssText =
  'position:fixed;top:20px;left:20px;width:320px;background:#1a1a2e;border:2px solid #9b59b6;border-radius:12px;padding:18px;z-index:999999;font-family:"Segoe UI",Arial,sans-serif;box-shadow:0 4px 20px rgba(0,0,0,0.5);color:#fff;';
...
panel.innerHTML = `
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;border-bottom:1px solid #333;padding-bottom:10px;">
    ...
    <span id="panel-status" style="font-size:12px;background:#e74c3c;color:#fff;padding:2px 8px;border-radius:10px;">监控�?/span>
  </div>
  <div style="font-size:13px;line-height:1.8;">
    <div>🔍 状态：�?暂时售罄</div>
    <div>🔄 已检查：<span id="check-count">0</span> �?/div>
    <div>�?已运行：<span id="run-time">0s</span></div>
    <div>📅 下次补货�?span style="color:#f1c40f;font-weight:bold;">06�?9�?10:00</span></div>
    <div>⏱️ 检测频率：�?30 �?/div>
  </div>
...
function checkButton() {
  ...
  // 查找 Lite 套餐的购买按�?  const btns = document.querySelectorAll('button, a, div[class*="btn"], div[role="button"]');

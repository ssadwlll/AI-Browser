// @name: 文本选中复制
// @description: 解除网站不允许复制的限制，文本选中后点击复制按钮即可复制。支持百度文库、道客巴巴、腾讯文档、豆丁网、知乎、CSDN、简书、掘金、B站、小红书、起点、纵横等60+网站
// @version: 6.2.10
// @urlPattern: *
(function () {
    'use strict';
    ...
    const website$u = {
      ...
      init: () => {
        ...
        fetch('https://res3.doc88.com/resources/js/modules/main-v2.min.js?v=3.55')
          .then(r => r.text())
          .then(text => {
            const result = /\("#cp_textarea"\).val\(([\S]*?)\);/.exec(text);
            if (result) path = result[1];
          })
          .catch(() => {});
        window.addEventListener("load", () => {
          const cpFn = window.copyText.toString();
    ...
    const website$p = {
      ...
      getSelectedText: function() {
        ...
        const sheetStatus = window.SpreadsheetApp ? window.SpreadsheetApp.permissions ? window.SpreadsheetApp.permissions.sheetStatus : window.SpreadsheetApp.sheetStatus : null;
    ...
    const website$o = {
      ...
      init: function() {
        let webpackJsonp = window.webpackJsonp;
    ...
    const website$a = {
      ...
      init: function() {
        ...
        let canvasDataGroup = [];
        const originObject = {
          context2DPrototype: window.document.createElement("canvas").getContext("2d").__proto__
        };
      ...
      getSelectedText: () => {
        ...
        try {
          const elements = window.document.querySelectorAll("#app > div");
    ...
    const website$4 = {
      ...
      getSelectedText: function() {
        if (window.docinReader && window.docinReader.st) {
          return window.docinReader.st;
        }
        return "";
      }
    };

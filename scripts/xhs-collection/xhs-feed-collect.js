...
async function behaviorLoop() {
  ...
  while (collectionRunning) {
    ...
    // 随机取一条（交换尾部+pop，O(1)删除，避免总按顺序访问同一关键词）
    ...
    // 导航前：当前页面鼠标+滚动（产生 collect 事件）
    await browserSimulate();

    // 导航到笔记详情页（产生 page_view + metrics_report + collect 事件）
    ...
    if (navResult.ok) {
      ...
      await sleep(randomDelay(1000, 2000));
      await browserSimulate();
      await sleep(randomDelay(3000, 6000));  // 阅读停留（关键：产生停留时长）
      await browserSimulate();
      await sleep(randomDelay(800, 1500));
      await browserSimulate();
      ...
      const backResult = await browserNavigate(searchUrl, randomDelay(2000, 4000));
      if (!backResult.ok) {
        log(`  [行为] 返回搜索页失败（不影响采集）`);
      }
      await browserSimulate();
    } else {
      log(`  [行为] 导航失败: ${navResult.error || '未知'}，在当前页面模拟`);
      await browserSimulate();
    }

    behaviorNoteCount++;

    // 每 8 条额外休息（参考验证过的采集脚本 batchPause.every=8, extraSleep=8000）
...
async function fetchFeed(noteId, xsecToken) {
  ...
  const bodyStr = JSON.stringify(body);
  // 纯 Node.js httpPost（不使用 browserFetch，浏览器专用于行为线程）
  ...
  for (let i = 0; i < notes.length; i++) {
    ...
    if (resp.code === 0) {
      ...
      if (retry.code === 0) {
        ...
        fail++;
        consecutiveFail++;
        failures.push({ noteId: note.noteId, xsecToken: note.xsecToken, code: retry.code, msg: retry.msg || '300015重试后失败' });
        log(`  [详情] 300015 重试返回其他错误: code=${retry.code}`);
        if (consecutiveFail >= 3) {
          log(`  [详情] 连续 ${consecutiveFail} 次失败，暂停 ${CONSECUTIVE_FAIL_PAUSE / 1000}s`);
          await sleep(CONSECUTIVE_FAIL_PAUSE);
          consecutiveFail = 0;
...
async function main() {
  ...
  log(`详情API: edith.xiaohongshu.com feed (纯 Node.js httpPost + 动态 XYS_ x-s + 动态 x-s-common)`);
  if (signServerAvailable) {
    log(`  签名服务: ${SIGN_SERVER_URL} ✅ 已连接（启动时cookie同步 + 行为线程后台运行）`);
